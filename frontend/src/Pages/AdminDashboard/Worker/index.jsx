...
const Worker = ({ onTotalWorkersChange }) => {
  ...
  const openEditModal = (worker) => {
    ...
    setFormData({
      ...
      services: worker.services?.[0]?.name || worker.services?.[0] || worker.services || "",
    });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setWorkers((prev) =>
        prev.map((w) => (w._id === currentWorker._id ? response.data.worker : w))
      );
          ...
          {filteredWorkers.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredWorkers.map((worker) => {
                const avatarUrl = profileMap[worker._id];
                return (
                  <div
                    key={worker._id}
                        ...
                        <div className="relative">
                          {avatarUrl ? (
                            <img
                              src={avatarUrl}
                              alt={worker.fullName}
                              className="w-16 h-16 rounded-2xl object-cover shadow-lg border-2 border-[#10b981]"
                            />
                          ) : (
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      <input
                        ...
                        readOnly
                        title="Services cannot be edited"
                        className="w-full px-4 py-2 pr-10 border-2 border-[#d1fae5] rounded-lg bg-gray-50 text-gray-500 cursor-not-allowed focus:outline-none"
                      />
                      <span
                    ...
                    <p className="text-xs text-gray-400 mt-1">Services cannot be edited</p>
                  </div>
                </div>
              </div>
